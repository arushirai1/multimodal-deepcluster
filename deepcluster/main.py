...
def main(args):
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        features = compute_features(dataloader, model, len(part_tr))
        ...
        mlp = list(model.classifier.children()) # line 159 removes the classifier layer
...
def compute_features(dataloader, model, N):
    ...
    for i, (input_tensor, _) in enumerate(dataloader):
        ...
        torch.no_grad()
        input_var = torch.autograd.Variable(input_tensor) #, volatile=True) #input_var = torch.autograd.Variable(input_tensor.cuda(), volatile=True)
        s_aux=time.time()
        print(i, s_aux)
